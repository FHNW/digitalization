define(["core/str", "jquery", "theme_bootstrapbase/bootstrap"], function(
  str,
  $
) {
  // var tooltip = $.fn.tooltip.noConflict();
  var NEBIS_URL = "http://recherche.nebis.ch/";
  var SWISSBIB_URL = "https://baselbern.swissbib.ch/";
  var module = {
    init: function() {
      // customize link
      $("#id_library").change(function() {
        var value = $(this).val();
        if (value == 12) {
          $("#library_search_link").remove();
          $("#id_library_url").before(
            $("<a />")
              .attr("id", "library_search_link")
              .attr("href", SWISSBIB_URL)
              .attr("target", "_blank")
          );
        } else {
          $("#library_search_link").remove();
      ...
      $("#id_back_to_automatic").click(function() {
        ...
        $("#id_author, #id_atitle, #id_title, #id_pub_date, #id_pages").each(
          function() {
            if ($(this).val() === "") {
              $(this).val(" ");
            }
      ...
      var warningPresent = str.get_string(
        "warning_submit_order",
        "digitalization"
      );
      ...
      var helpPresent = str.get_string(
        "to_library_catalogue",
        "digitalization"
      );
